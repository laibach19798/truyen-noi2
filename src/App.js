...
export default function StoryListenerApp() {
  ...
  // TTS MODE: 'edge' (Hoài My/Nam Minh) hoặc 'web' (Google nhanh)
  ...
  const prefetchedAudioRef = useRef({}); // Cache audio đã tải
  ...
  const parseEpub = async (file) => {
    ...
    if (!window.JSZip) throw new Error('Không thể tải thư viện đọc EPUB');
  ...
  const handleFileUpload = async (event) => {
    ...
    setErrorMsg('');
    prefetchedAudioRef.current = {}; // Clear cache
    try {
      let newChapters = [];
      const fileName = file.name.toLowerCase();
      if (fileName.endsWith('.epub')) {
        newChapters = await parseEpub(file);
        if (newChapters.length === 0) throw new Error('File EPUB không có nội dung');
  ...
  // ============ EDGE TTS ============
  const fetchEdgeTTSAudio = async (text, voice = voiceGender, rate = speechRate) => {
    // Kiểm tra cache trước
    const cacheKey = `${text.substring(0, 50)}_${voice}_${rate}`;
    if (prefetchedAudioRef.current[cacheKey]) {
      return prefetchedAudioRef.current[cacheKey];
    }
    ...
    if (!response.ok) {
      throw new Error('Không kết nối được Edge TTS server');
    }
    
    const blob = await response.blob();
    const url = URL.createObjectURL(blob);
    prefetchedAudioRef.current[cacheKey] = url;
    return url;
  };
  ...
  const prefetchNextChapter = async () => {
    ...
    if (nextIdx >= chaptersRef.current.length) return;
    
    const nextChapter = chaptersRef.current[nextIdx];
    if (!nextChapter) return;
    
    try {
      await fetchEdgeTTSAudio(nextChapter.content);
      console.log('✅ Prefetched chương', nextIdx + 1);
    } catch (e) {
  ...
  // Highlight chữ theo audio time (Edge TTS)
  const startWordHighlightByTime = (text, totalDuration) => {
    const wordList = splitIntoWords(text);
    setWords(wordList);
    
    // Tính thời gian mỗi từ dựa trên độ dài
    const totalChars = text.length;
    let charSum = 0;
    const wordTimings = wordList.map(word => {
      const startTime = (charSum / totalChars) * totalDuration;
      charSum += word.length;
      return startTime;
    });
    
    // Update theo thời gian audio
    const checkProgress = () => {
      ...
      for (let i = wordTimings.length - 1; i >= 0; i--) {
        if (wordTimings[i] <= currentTime) {
          currentIdx = i;
          break;
        }
      }
      setCurrentWordIndex(currentIdx);
      setTimeout(() => scrollToWord(currentIdx), 50);
  ...
  const playEdgeTTS = async (text) => {
    ...
    setErrorMsg('');
    try {
      const audioUrl = await fetchEdgeTTSAudio(text);
      ...
      audioRef.current.playbackRate = 1; // Edge TTS đã apply rate
      
      audioRef.current.onloadedmetadata = () => {
        startWordHighlightByTime(text, audioRef.current.duration);
      };
        ...
        if (autoNextRef.current) {
          ...
          if (nextIdx < chaptersRef.current.length) {
            setTimeout(() => {
              ...
              setTimeout(() => {
                playEdgeTTS(chaptersRef.current[nextIdx].content);
              }, 500);
            }, 300);
          } else {
            setIsPlaying(false);
      ...
      // Prefetch chương kế tiếp
      setTimeout(prefetchNextChapter, 2000);
      
    } catch (error) {
      setIsLoadingAudio(false);
      setIsPlaying(false);
      setErrorMsg('Không kết nối Edge TTS. Đang chuyển sang giọng nhanh...');
      // Fallback sang Web Speech
      setTimeout(() => {
        setTtsMode('web');
        playWebSpeech(text);
      }, 1000);
    ...
    utteranceRef.current.onboundary = (event) => {
      if (event.name === 'word' || event.name === undefined) {
        ...
        for (let i = 0; i < wordList.length; i++) {
          if (currentChar >= charIndex) { wordIdx = i; break; }
    ...
    utteranceRef.current.onend = () => {
      ...
      if (autoNextRef.current) {
        ...
        if (nextIdx < chaptersRef.current.length) {
          setTimeout(() => {
            setCurrentChapterIndex(nextIdx);
            setTimeout(() => playWebSpeech(chaptersRef.current[nextIdx].content), 800);
          }, 300);
        } else {
          setIsPlaying(false);
        }
  ...
  const togglePlay = async () => {
    ...
    if (isPlaying || isLoadingAudio) {
      ...
      if (ttsMode === 'edge') {
        await playEdgeTTS(currentChapter.content);
      } else {
        playWebSpeech(currentChapter.content);
      }
    }
  };
  ...
  const loadFromWeb = async () => {
    ...
    try {
      ...
      setErrorMsg('Không thể tải từ URL');
    }
          ...
          <div className="flex gap-2">
            ...
            >
              ...
              <div className="text-left">
                ...
                <div className="text-xs opacity-80">Giọng hay (chờ 5-10s)</div>
              </div>
            </button>
            <button
              onClick={() => { stopAllAudio(); setTtsMode('web'); }}
              className={`flex-1 py-3 rounded-xl transition-all flex items-center justify-center gap-2 ${
        ...
        {chapters.length === 0 && (
          <div className={`${theme.card} border rounded-2xl p-6 md:p-8 mb-6`}>
            ...
            <button
              ...
              className={`w-full ${theme.buttonPrimary} font-semibold py-4 rounded-xl transition-all shadow-lg mb-3 disabled:opacity-50`}
            >
              {isLoadingFile ? (
            ...
            <div className="flex gap-2 mt-4">
              <input
                ...
                placeholder="Hoặc dán link truyện..."
